...
hash_tree() {
    ...
    done < <(if [ -n "${2:-}" ]; then cat -- "$2"; else find "$1" -path "$1/._MANIFESTS" -prune -o -type d -print0 2>> "$FAILLOG" || echo "TRAVERSAL-FAILED $1" >> "$FAILLOG"; fi)
...
action_metadata() {
    ...
    if [ -n "$DROP" ]; then mkdir -p -- "$MANIFEST_DIR" && cp -f -- "$OUT/._manifest-metadata.jsonl" "$OUT/._manifest-hashrecord.sha256" "$OUT/._manifest-header.meta" "$MANIFEST_DIR/" && { chown -R -- "${SUDO_UID:-0}:${SUDO_GID:-0}" "$DROP/DISKUTILS" 2>/dev/null || :; echo "${GREEN}[+]${NC} Desktop copy: $MANIFEST_DIR"; rm -rf -- "$OUT"; } || echo "${YELLOW}[!]${NC} No Desktop copy written (copy failed); files are in $OUT."; elif [ "$SAVE_DRIVE" -eq 1 ]; then echo ""; echo "${GREEN}[+]${NC} Manifest saved to $METADIR."; rm -rf -- "$OUT"; else echo "${YELLOW}[!]${NC} No persistent manifest destination available; files are in $OUT."; fi
    echo ""
    [ "$COMPLETE" -eq 1 ]
}
