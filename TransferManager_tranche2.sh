...
action_metadata() {
    ...
    awk -v RS='\036' -v FS='\037' -v FE="$(grep -c '' "$OUT/find.err" || :)" "$LEDGER_AWK"'function esc(s,  c) { gsub(/\\/,"\\\\",s); gsub(/"/,"\\\"",s); if (s ~ /[[:cntrl:]]/) for (c=1;c<32;c++) gsub(sprintf("%c",c), sprintf("\\u%04x",c), s); return s }
        $0=="SRCFAIL" { src++; next }
        NF==3 { v=$3; sub(/^[ \t]+/,"",v); M[$1]=v; next }
        NF==14 { t=$5; q=lesc($3); D=$2; g=$2 "/" $3; p=esc($13); l=($1=="l"); d=($1=="d")
