...
action_metadata() {
    ...
    R="$METADIR" awk -v D="$(TZ=UTC date +%Y-%m-%dT%H:%M:%SZ)" -v MV="$(file --version | head -1)" 'BEGIN{R=ENVIRON["R"]; FS="\037"; gsub(/\\/,"\\\\",R); gsub(/"/,"\\\"",R); if(R ~ /[\001-\037]/) for(c=1;c<32;c++) gsub(sprintf("%c",c), sprintf("\\u%04x",c), R)} $1=="\001"{REJ=$3; SRC=$4; next} {A[++n]=$2; B[n]=$3} END{printf "{\"type\":\"manifest\",\"version\":1,\"tool\":\"TransferManager\",\"root\":\"%s\",\"created\":\"%s\",\"hash\":\"sha256\",\"unicode\":\"none\",\"magic\":\"%s\",\"complete\":%s,\"rejected\":%d,\"collection_errors\":%d,\"records\":%d}\n", R, D, MV, (REJ+0||SRC+0?"false":"true"), REJ+0, SRC+0, n; for (i=1;i<=n;i++) printf "{\"num\":%d,%s\n", i, substr(A[i],2)}' > "$OUT/._manifest-metadata.jsonl"
    ...
    echo ""
    [[ $(head -n 1 -- "$OUT/._manifest-metadata.jsonl") == *'"complete":true'* ]] && { COMPLETE=1; echo "${CR}${GREEN}[+] Success!${NC} Wrote $N records."; } || { COMPLETE=0; echo "${CR}${DEEP_RED}[!] Incomplete!${NC} Wrote $N records, but the manifest header says \"complete\":false."; head -n 20 -- "$OUT/find.err" | cat -v >&2; }
