...
# Preflight: exits unless every named tool is installed (rsync must be >= 3.1 for --info=progress2)
need() { local t; for t; do command -v "$t" >/dev/null || { echo "${DEEP_RED}[ERROR] '$t' is required but not installed.${NC}"; exit 1; }; done; [[ " $* " != *" rsync "* ]] || rsync --version | awk 'NR==1 {split($3,v,"."); ok=(v[1]>3 || (v[1]==3 && v[2]>=1))} END {exit !ok}' || { echo "${DEEP_RED}[ERROR] rsync 3.1 or newer is required.${NC}"; exit 1; }; }
need flock
...
prompt_path() {
    ...
    printf -v "$1" '%s' "$val"
    case $1 in SOURCE|DEST|HASHDIR|METADIR) [ -z "${PATHS:-}" ] || declare -p "$1" >> "$PATHS" ;; esac
}
...
function lparse(s) { sub(/^\\/,"",s); LS=(substr(s,1,8)=="SYMLINK:"); if (LS) s=substr(s,9); LH=tolower(substr(s,1,64)); LN=substr(s,67); gsub(/\r/,"\\r",LN); return (length(LH) == 64 && LH !~ /[^0-9a-f]/ && substr(s,65,2) ~ /^ [ *]$/) }
'
...
# Prints a new temp ledger path beside $1 for the atomic replace. mktemp creates it O_EXCL under an unpredictable name, so a
# planted symlink can't occupy it as it could the fixed .tmp path; it gets the mode a plain > redirect would have given.
ledger_tmp() { local t; t=$(mktemp -p "$(dirname -- "$1")" "${1##*/}.XXXXXX") && chmod "$(printf %o $((0666 & ~$(umask))))" -- "$t" && printf '%s\n' "$t"; }
...
process_hash_directory() {
	...
	[ ! -L "$CHECKSUM_FILE" ] && [ ! -L "$CHECKSUM_FILE.tmp" ] && [ ! -L "$CHECKSUM_FILE.bak" ] || { echo "FAILED $SUBDIR (a ._localhash.sha256 ledger path is a symlink; folder skipped)" >> "$FAILLOG"; exit 0; }
	...
	while IFS= read -r -d '' FILE; do
	    ...
	    SHOW="${SUBDIR##*/}/$F_NAME"; printf '%s%s  ->%s Hashing: %.*s%s' "$CR" "$GREEN" "$NC" "$((COLS-15))" "${SHOW//[[:cntrl:]]/?}" "$EOL"
	...
	if [ "$LEDGER_CHANGED" -gt 0 ]; then
	    LEDGER_TMP=$(ledger_tmp "$CHECKSUM_FILE")
	    {
	    ...
	    } | LC_ALL=C sort -k 2 > "$LEDGER_TMP" && mv -- "$LEDGER_TMP" "$CHECKSUM_FILE" || { rm -f -- "$LEDGER_TMP"; exit 1; }
	fi
    )
}
...
hash_tree() {
    local SUBDIR DROP FAILLOG SHOWN NDIRS=0; FAILLOG=$(mktemp -p /run transfermanager-hash-errors.XXXXXX) || { echo "${DEEP_RED}[-] Error:${NC} Cannot create the hash error log under /run."; return 1; }
    echo ""
    ...
    grep -q '[^[:space:]]' "$FAILLOG" && { SHOWN=; DROP=$(desktop_dir) && user_write "$DROP" ._hash_errors.log < "$FAILLOG" 2>/dev/null && SHOWN=$(realpath -e -- "$DROP/DISKUTILS/._hash_errors.log"); echo "${CR}${DEEP_RED}[!] Incomplete!${NC} $(grep -c '^UNREADABLE' "$FAILLOG" || :) unreadable, $(grep -c '^FAILED' "$FAILLOG" || :) failed, $(grep -c '^TRAVERSAL-FAILED' "$FAILLOG" || :) traversal errors, $(grep -v '^\(UNREADABLE\|FAILED\|TRAVERSAL-FAILED\) ' "$FAILLOG" | grep -c '[^[:space:]]' || :) other messages. See ${SHOWN:-$FAILLOG}."; [ -z "$SHOWN" ] || rm -f -- "$FAILLOG"; return 1; } || { [ -z "${2:-}" ] && echo "" && echo "${CR}${GREEN}[+] Success!${NC} All directory hashes updated and sorted."; rm -f "$FAILLOG"; }
}
...
action_copy() {
    ...
    if [ -L "$SOURCE" ] || [ -f "$SOURCE" ]; then
        ...
        SRC_HASH=; SRC_LEDGER="$(dirname -- "$SOURCE")/._localhash.sha256"; ledger_esc SRC_KEY "$(basename -- "$SOURCE")"; [ -L "$SOURCE" ] || { [ -f "$SRC_LEDGER" ] && [ ! -L "$SRC_LEDGER" ] && [ ! "$SOURCE" -nt "$SRC_LEDGER" ] && SRC_HASH=$(LD_T="$SRC_KEY" awk "$LEDGER_AWK"'lparse($0) && !LS && LN == ENVIRON["LD_T"] {h=LH} END {print h}' "$SRC_LEDGER"); [ -n "$SRC_HASH" ] || echo "${YELLOW}[!]${NC} Warning: '$SOURCE' has no current entry in its source ledger; its hash will be computed from the copy, not the source."; }
        
        # Isolate directory navigation in a subshell for safety, append new hash and sort deterministically
        (
            cd "$TARGET_DIR" || exit 1
            CHECKSUM_FILE="._localhash.sha256"
            [ ! -L "$CHECKSUM_FILE" ] && [ ! -L "$CHECKSUM_FILE.tmp" ] && [ ! -L "$CHECKSUM_FILE.bak" ] || { echo "${DEEP_RED}[-] Error:${NC} A ._localhash.sha256 ledger path in the destination folder is a symlink; the file was copied but its hash was not recorded."; exit 1; }
            echo "${GREEN}  ->${NC} Hashing file: ${F_NAME//[[:cntrl:]]/?}"
            ...
            LEDGER_TMP=$(ledger_tmp "$CHECKSUM_FILE") && { ledger_drop "$CHECKSUM_FILE" "$F_NAME"; printf '%s\n' "$NEW_HASH_LINE"; } > "$LEDGER_TMP" && mv -- "$LEDGER_TMP" "$CHECKSUM_FILE"
            LEDGER_TMP=$(ledger_tmp "$CHECKSUM_FILE") && LC_ALL=C sort -k 2 "$CHECKSUM_FILE" > "$LEDGER_TMP" && mv -- "$LEDGER_TMP" "$CHECKSUM_FILE"
            echo ""
    ...
    # DIRECTORY COPY LOGIC ####################################################################################
        ...
        echo ""; echo "";
        need rsync stdbuf; echo "${YELLOW}[!]${NC} Directory detected. Using a single rsync -aHAXS pass, then hashing."
        ...
        CHECKSUM=; [ -z "$(find "$DEST" -mindepth 1 -print -quit 2>/dev/null)" ] || { read -r -p "Hash verification for pre-existing destination files (default skips files with identical size and timestamp)? [y/N]: " CHECKSUM || CHECKSUM=; [[ "$CHECKSUM" =~ ^[Yy] ]] || CHECKSUM=; }
        rsync -aHAXSh ${CHECKSUM:+"--checksum"} --partial --info=progress2 -- "${SOURCE%/}/" "$DEST/" 2>"$COPY_TMP/errors" | stdbuf -o0 tr '\r' '\n' | while IFS= read -r LINE; do printf '%s%s' "$CR" "$LINE"; done || { echo ""; cat -- "$COPY_TMP/errors" >&2; echo "${DEEP_RED}[-] Error:${NC} rsync failed; the copy is incomplete."; exit 1; }
...
action_verify() {
    ...
    while IFS= read -r -d '' D; do
        [ -z "${DONE["$D"]-}" ] || continue
        ST=0 OUT=''; printf '%s%s  ->%s Verifying: %.*s%s' "$CR" "$GREEN" "$NC" "$((COLS-18))" "${D//[[:cntrl:]]/?}" "$EOL"
        if [ ! -f "$D/._localhash.sha256" ]; then
            FL=$(find "$D" -maxdepth 1 \( -type f -o -type l \) ! \( "${EXCLUDE_FILES[@]}" \) -print -quit) || ST=$?
            [ -z "$FL" ] || OUT="\"${D//\"/\"\"}\",NO LEDGER,,,"
        elif [ -L "$D/._localhash.sha256" ]; then OUT="\"${D//\"/\"\"}\",CORRUPT LEDGER,ledger is a symlink,,"
        else
            ...
            [ ! -s "$STATE.err" ] || { cat -v -- "$STATE.err" >&2; OUT="${OUT:+$OUT$'\n'}\"${D//\"/\"\"}\",UNREADABLE,,,"; }
        fi
        [ "$ST" = 0 ] || OUT="\"${D//\"/\"\"}\",ERROR,,,"
...
action_metadata() {
    need python3 file; prompt_path METADIR "Enter the drive location to export metadata manifest: " dir; METADIR=${METADIR%/}
    ...
    R="$METADIR" awk -v D="$(TZ=UTC date +%Y-%m-%dT%H:%M:%SZ)" -v MV="$(file --version | head -1)" 'BEGIN{R=ENVIRON["R"]; FS="\037"; gsub(/\\/,"\\\\",R); gsub(/"/,"\\\"",R); if(R ~ /[\001-\037]/) for(c=1;c<32;c++) gsub(sprintf("%c",c), sprintf("\\u%04x",c), R)} $1=="\001"{REJ=$3; SRC=$4; next} {A[++n]=$2; B[n]=$3; if (index($2,"\"sha256\":null")) U++} END{printf "{\"type\":\"manifest\",\"version\":1,\"tool\":\"TransferManager\",\"root\":\"%s\",\"created\":\"%s\",\"hash\":\"sha256\",\"unicode\":\"none\",\"magic\":\"%s\",\"complete\":%s,\"rejected\":%d,\"collection_errors\":%d,\"unhashed\":%d,\"records\":%d}\n", R, D, MV, (REJ+0||SRC+0||U+0?"false":"true"), REJ+0, SRC+0, U+0, n; for (i=1;i<=n;i++) printf "{\"num\":%d,%s\n", i, substr(A[i],2)}' > "$OUT/._manifest-metadata.jsonl"
    ...
    STAMP=$(TZ=UTC date +%Y%m%dT%H%M%SZ); N=$(( $(wc -l < "$OUT/._manifest-metadata.jsonl") - 1 )); printf '## Header Generated by TransferManager ## \ngenerated         : %s\nrun_stamp         : %s\nhost              : %s\nroot_directory    : %s\nmetadata_scope    : %s\nmetadata_records  : %s\nhashed_records    : %s\nsymlinks_skipped  : %s\nunhashed_records  : %s\nlast_operation    : metadata\ninvoked_by        : %s\nhistory_dir       : ._MANIFESTS/%s\nmetadata_sha256   : %s\nhashrecord_sha256 : %s\nmetadata_header   : %s\nverify_all        : cd %q && sha256sum -c --quiet %q\nverify_folder     : cd <dir> && grep -Ev "^[\]?SYMLINK:" ._localhash.sha256 | sha256sum -c --quiet\n' "$(date -Is)" "$STAMP" "$(hostname)" "$MANIROOT" "$METADIR" "$N" "$(wc -l < "$OUT/._manifest-hashrecord.sha256")" "$(grep -c '"type":"symlink"' "$OUT/._manifest-metadata.jsonl" || :)" "$(grep -c '"sha256":null' "$OUT/._manifest-metadata.jsonl" || :)" "${SUDO_USER:-$(id -un)}" "$STAMP" "$(sha256sum < "$OUT/._manifest-metadata.jsonl" | cut -d' ' -f1)" "$CHECKSUM_HASH" "$(head -1 -- "$OUT/._manifest-metadata.jsonl")" "$MANIROOT" "${MANIFEST_DIR:-$([ "$SAVE_DRIVE" -eq 1 ] && printf %s "$MANIROOT/._MANIFESTS/$STAMP" || printf %s "$OUT")}/._manifest-hashrecord.sha256" > "$OUT/._manifest-header.meta"
    if [ "$SAVE_DRIVE" -eq 1 ]; then
        mkdir -p -- "$MANIROOT/._MANIFESTS/$STAMP" && cp -f -- "$OUT/._manifest-metadata.jsonl" "$OUT/._manifest-hashrecord.sha256" "$OUT/._manifest-header.meta" "$MANIROOT/._MANIFESTS/$STAMP/" || { echo "${YELLOW}[!]${NC} Historical copy under $MANIROOT/._MANIFESTS/$STAMP failed."; SAVE_DRIVE=0; }
    fi
    echo ""
    [[ $(head -n 1 -- "$OUT/._manifest-metadata.jsonl") == *'"complete":true'* ]] && { COMPLETE=1; echo "${CR}${GREEN}[+] Success!${NC} Wrote $N records."; } || { COMPLETE=0; echo "${CR}${DEEP_RED}[!] Incomplete!${NC} Wrote $N records, but the manifest header says \"complete\":false ($(grep -c '"sha256":null' "$OUT/._manifest-metadata.jsonl" || :) without a current hash)."; head -n 20 -- "$OUT/find.err" | cat -v >&2; }
    if [ -n "$DROP" ]; then user_write "$DROP" MANIFEST/._manifest-metadata.jsonl < "$OUT/._manifest-metadata.jsonl" && user_write "$DROP" MANIFEST/._manifest-hashrecord.sha256 < "$OUT/._manifest-hashrecord.sha256" && user_write "$DROP" MANIFEST/._manifest-header.meta < "$OUT/._manifest-header.meta" && { echo "${GREEN}[+]${NC} Desktop copy: $MANIFEST_DIR"; rm -rf -- "$OUT"; } || { trap - EXIT; echo "${YELLOW}[!]${NC} No Desktop copy written (copy failed); files are in $OUT."; }; elif [ "$SAVE_DRIVE" -eq 1 ]; then echo ""; echo "${GREEN}[+]${NC} Manifest saved to $MANIROOT/._MANIFESTS/$STAMP."; rm -rf -- "$OUT"; else trap - EXIT; echo "${YELLOW}[!]${NC} No persistent manifest destination available; files are in $OUT."; fi
    echo ""
    [ "$COMPLETE" -eq 1 ]
}

PATHS=$(mktemp -p /run transfermanager-paths.XXXXXX) || { echo "${DEEP_RED}[ERROR] Cannot create the session file under /run.${NC}"; exit 1; }; trap 'rm -f -- "$PATHS"' EXIT
while :; do
display_banner
...
case "${MENU_CHOICE-5}" in
    0) ( set -e; action_copy ) ;;
    1) ( set -e; action_hash ) ;;
    2) ( set -e; action_rehash ) ;;
    3) ( set -e; action_metadata ) ;;
    4) ( set -e; action_verify ) ;;
    5) if [ -t 1 ]; then clear 2>/dev/null || true; fi; echo "Operation canceled."; echo ""; exit 0 ;;
esac
RC=$?; set -e; . "$PATHS"
echo ""
while :; do
    read -r -p "Return to the main menu? [Y/n]: " REPLY || exit "$RC"
    case "$REPLY" in ""|[Yy]|[Yy][Ee][Ss]) break ;; [Nn]|[Nn][Oo]) exit "$RC" ;; esac
done
if [ -t 1 ]; then clear 2>/dev/null || true; fi
done
